...
def Analysis(PicName, location):
    ...
    while local < Key_Value_pair and running == 1:
        ...
        if NowRowKey[local] == '左键':
            ...
            temp_key = StartKey.split(',')
            print(f'1:{temp_key[0]} 2:{temp_key[1]} 3:{temp_key[2]}')
            if CurrentROW == int(temp_key[0]) or CurrentROW == int(temp_key[1]) or CurrentROW == int(temp_key[2]):
                # with open(r'./Source/temp.txt', 'r', encoding='utf-8') as fp:
                #     line_txt = fp.readlines()
                fp = open(r'./Source/temp.txt', 'r', encoding='utf-8')
                line_txt = fp.readlines()
                lines = [line.strip('\n') for line in line_txt]
                print(f'txt_cont: {lines}')
                print(f'count: {len(lines)}')
                print(f'第 {times+1} 次: {str(lines[times])}')
...
def ThreadShowUIAndManageEvent():
    ...
    def UpdataCurrentXls():  # 当前表格重定向
        ...
        if '.xls' not in FilesList:
            ...
            # tkinter.messagebox.showinfo(title='PyRPA: ', message='！路径' + WorkPath +
    ...
    Lab = tk.Label(Top, text="ExcelTemp", font=("宋体", 14), fg=g_fg)
    ...
    # 使用tk样式 butt = tk.Button(Top, text="保存并刷新", width=15, height=1, font=("方正姚体", 11), fg="#E8E8E8", relief=RIDGE,
    # command=UpdataCfg) butt.place(x=25, y=250, width=120)
